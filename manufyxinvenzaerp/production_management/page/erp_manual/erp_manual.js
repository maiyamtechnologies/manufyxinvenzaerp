...
const ERP_MANUAL_SALES_ORDER_CHILDREN = [
	...
	},
	{
		...
		fields: [
			...
			{ name: "Grade", note: "Must already exist in the <b>Material Grade</b> master — e.g. IS2062. Checked by name exactly as typed, so watch the spacing: IS2062 and IS 2062 are two different grades. A blank Grade is allowed only for groups other than Structurals and Plates, where it is mandatory. Create a missing one from the Material Masters card on the Manufyx workspace, or correct the sheet and load it again." },
			{ name: "Material Spec", note: "Column beside Grade. <b>Mandatory, together with Grade, on every Structurals and Plates row</b> — Verify lists each row missing either. Optional for other groups (Nuts and Bolts and the rest). When filled it must already exist in the <b>Material Spec</b> master, checked by name exactly as typed." },
			{ name: "Spec and Grade against the Item", note: "A filled-in Material Spec or Grade must be the one on that Material Code's <b>Item</b>. If it is not, Verify stops and lists each row: DUNO, Material Code, what the sheet asks for and what the Item has — <i>this material spec or grade is not in the Item master</i>. Create or correct the Item, set it as the Material Code, and Verify again. This matters because spec and grade travel from the Item onto Material Planning, Material Request, PO and PR: a sheet asking for E350 against an E250 Item would otherwise plan and buy E250. A blank spec or grade in the sheet is not checked." },
...
const ERP_MANUAL_ITEM_CHILDREN = [
	...
	},
	{
		...
		notes: [
			...
			"<b>Material Spec and Material Grade lock once set.</b> On an Item that already has transactions, " +
				"a blank spec or grade can still be filled in once, but a spec or grade that is set cannot be " +
				"changed — a different spec or grade is a different material, so create a new Item for it. " +
				"Filling a blank one stays possible because every Item in use had neither set when this rule arrived.",
...
const ERP_MANUAL_MATERIAL_PLANNING_CHILDREN = [
	...
	},
	{
		...
		steps: [
			...
			"<b>Check stock without dimensions</b> (tick box under Raw Materials Warehouse) — for Plates and Structurals the size no longer has to match. Any batch of the SAME item counts (never another item): the drawing's own size first, then other sizes, largest free first. Availability is the exact Kg, with no rounding to whole pieces. The row gets <b>Reserve stock without dimensions</b>, shows the batch's own size, and its Nos is that Kg as a fraction of one piece of the batch (settled into whole pieces at transfer). What stock can't cover goes to Material Mapping. Rows already reserved stay as they are when you check again with the box ticked.",
		],
		calcs: [
			{
		...
		notes: [
			"<b>A crumb left in a batch is not stock.</b> Splitting one batch across several drawings can leave a fraction of a Kg once its pieces are all spoken for. That remainder is no longer offered to the next drawing — it would make a 0 Nos row and split that drawing across two batches for nothing. A batch counts as having stock only while what is left is worth at least a thousandth of one of its pieces; a genuine part-piece still counts.",
			"<b>Checking again after a purchase.</b> Stock is matched first — including what the receipt brought in. A requirement still on a Material Request that has quantity left to receive keeps its Unavailable Items row only for the part stock can't cover; once the Material Request is fully received it is no longer held back, so nothing already received is asked for again.",
	...
	},
	{
		...
		buttons: [
			{ name: "Allocate from Purchase Receipt (grid button)", note: "Pick a submitted Purchase Receipt: the batches it brought in are mapped onto this plan's requirements that nothing covers yet — same item, any size (the drawing's own size first), exact Kg. A requirement already waiting here as <b>Not Mapped</b> has its own row filled in (the part a batch cannot cover stays as its own Not Mapped row); anything else gets a new row. Each row keeps the requirement's size and carries the batch's on the Batch fields, with <b>Reserve stock without dimensions</b> ticked. Use it when a receipt's automatic allocation did not map, or after re-checking stock. Stock reserved by any plan, or already on a row of this plan, is never used again, and a batch already in Exact Match is skipped. Covered requirements come off Unavailable Items. Nothing is reserved — press Reserve afterwards." },
...
const ERP_MANUAL_PRODUCTION_PLAN_CHILDREN = [
	...
	},
	{
		...
		fields: [
			...
			{ name: "Supplier/Contractor", note: "Who carries out this operation. A <b>Subcontractor</b> row lists Suppliers; an <b>Internal Jobcard</b> row lists Contractors, from the <b>Contractor</b> master (name, address, contact number, remarks). Changing Work Type clears it, since a Supplier cannot do an internal job. <b>Mandatory:</b> the plan cannot be saved from the form, or submitted, until every row has one. A plan made by Make Production Plan arrives without them, because nobody could know who does each operation yet, so fill them in before saving. Each operation's Supplier Operation Entry then carries its own row's Supplier or Contractor, and the Job Work Order's Operations table shows it." },
...
const ERP_MANUAL_JOB_WORK_ORDER_CHILDREN = [
	{
		...
		steps: [
			...
			"The Operations tab shows a live summary table — Seq, Operation, Supplier/Contractor, Status, Overall Qty, Available to Consume, Total Consumed, Difference, Entry, Drawings. Click any operation's name (shown in blue, underlined) to jump straight into that Supplier Operation Entry.",
		...
		notes: [
			...
			"<b>Total Estimated Taxes</b> and <b>Grand Total</b> are hidden. Both are ERPNext pricing a subcontracting order its own way, from item rates and the Taxes table. A job work order raised from a Production Plan is priced per drawing in Kg through the Drawing Items table, and the Taxes table is not used — so Total Estimated Taxes read 0 on every order and Grand Total repeated a figure that left out the job work being paid for. The fields and any data in them are untouched; they are only hidden from the form.",
...
const ERP_MANUAL_SOE_CHILDREN = [
	{
		...
		fields: [
			...
			{ name: "Weight Summary", note: "Two columns: the Nos pair (Available to Consume NOS, Total Consumed NOS) on the left, <b>Available to Consume (Kg)</b> and <b>Total Consumed (Kg)</b> on the right. On operation 1 the Kg figure is what was actually transferred to the supplier; from operation 2 on it is what the previous operation consumed and handed forward." },
		...
		buttons: [
			{ name: "Add Pending Drawing to Log", note: "A direct button (not inside a group). Fills Consumption Log with one row per drawing for the quantity not yet logged: an empty log gets every drawing in full; with 3 of 5 already logged it adds one new row of 2; a drawing already fully logged is skipped, so clicking again adds nothing twice. For quick testing or data entry, not a normal production step — which is why it only appears when <b>Auto Purchase from Material Planning</b> is ticked in Manufyxinvenza Settings, the same switch that reveals the Auto Purchase section on Material Planning. Leave that off on a live site and this button is not there at all." },
		],
		notes: [
...
const ERP_MANUAL_INSPECTION_CHILDREN = [
	{
		...
		fields: [
			...
			{ name: "Inspection Entry — header", note: "Three columns: the source and round, the work references, and the parties (Sales Order, Customer, Supplier — or <b>Contractor</b> for an internal operation). Below them Status (Open/Working/Completed), Inspection Complete Date and Overall Remarks." },
			{ name: "Feedback and Rework Remarks — per row", note: "Each drawing row has its own <b>Feedback</b> (Ok / Not Ok) and <b>Rework Remarks</b>. Ok means nothing on that row is rejected, Not Ok means something is. Rework Remarks is mandatory on any row with a Rejected Qty. There is no Feedback or Rework Remarks to fill on the header any more: the entry's overall result is worked out from the rows (Not Ok if any row is Not Ok) and is what the Inspection Status report shows. Entries made before 2026-09-23 had their header Feedback copied onto every row, and their Rework Remarks onto the rejected rows. A Job Card inspection has no rows and keeps both on the header." },
		...
		steps: [
			"Click the dark <b>Create Inspection</b> button above the Consumption Log on the Supplier Operation Entry (save it first). The popup asks for the <b>Inspection Call Date</b> (today by default) and lists the drawings that will be inspected with their Completed NOS — whatever is pending in Inspection Items. <b>Create</b> logs the call and creates the Inspection Entry in one step; if the entry cannot be created, no call is logged either. The button is hidden while an inspection is still open — use <b>Inspection → View Inspection Entry</b> for that one, or the <b>Connections</b> tab, which lists every Inspection Entry of the operation.",
			"Enter Accepted Qty per drawing row — Rejected Qty fills in automatically. Then set that row's Feedback, and its Rework Remarks when anything is rejected.",
			"Every row needs Feedback before Status can be Completed — trying to complete without it names the rows still missing it and reverts Status.",
		...
		notes: [
			"Rework Remarks is mandatory on every row whose Rejected Qty is more than 0. The Inspection Status and Inspection Call Date fields on the Supplier Operation Entry's Inspection tab are hidden: the date is picked in the Create Inspection popup and the status is kept up to date in the background.",
	...
	},
	{
		...
		steps: [
			...
			"Record Accepted Qty and Rejected Qty per line, set each line's Feedback (Ok when nothing is rejected, Not Ok otherwise), add remarks, set Status to Completed and submit.",
...
const ERP_MANUAL_MATERIAL_ISSUE_PLAN_CHILDREN = [
	...
	},
	{
		...
		steps: [
			...
			"Switch to <b>Consolidate item for excess return plan</b> and measure the off-cut, one line per item — for the items that have one. A line whose Excess Kg (system) is zero has its boxes closed: nothing was left over, so there is nothing to measure. <b>Off-cut in several sizes:</b> the duplicate icon at the end of a Structurals or Plates line adds another Length / Width / NOS line for the same item (the bin icon removes it). The Difference is worked out on all sizes together, every size must be complete before Transfer, and each size is booked as its own excess return row. <b>Return NA:</b> tick it on a line whose off-cut is not coming back. It can only be ticked where the Excess Kg (system) is less than <b>Excess Return Not Applicable if Less Than (Kg)</b> in Manufyxinvenza Settings (1 Kg by default; 0 switches it off) — on every other line the box stays closed. Ticked, the dimension boxes close, no dimensions are asked for, nothing is declared to return, and that weight is written off with the job's Process Loss at the end (the Process Loss popup names it, and the Decision Log records who ticked it). Optional — leave it blank and only a rounding surplus is booked, as before. <b>Excess Kg (system) compares like with like</b>: a requirement filled from two batches of different sizes is counted once, a drawing needing the same item in two cut sizes keeps both, and the CNC leg is measured against its own share rather than the whole item's. Before that was true this tab reported six-figure shortfalls on plans whose mapping covered the requirement exactly.",
	...
	},
	{
		...
		steps: [
			...
			"<b>Completed also waits for material that never went out.</b> Nothing at the supplier is only half the question — a plan with raw material still sitting in stores is not finished either, and the two are measured at opposite ends of the journey. A plan once closed itself with 472.615 Kg of CNC material still in stores, because the batches that did travel balanced to exactly zero at the supplier; the Completed lock then hid the very button that would have sent it. If a plan will not complete and the supplier's side is clear, open <b>Transfer</b> and look for a leg still outstanding.",
		],
		calcs: [
			{
	...
	},
	{
		...
		fields: [
			...
			{ name: "Return Excess Entry", note: "Review quantities and enter a mandatory reason per row, then the return Stock Entry is created into the Finished Goods Warehouse. <b>One off-cut coming back as several sizes:</b> the duplicate icon at the end of a Structurals or Plates row adds another line under it asking only for Length, Width (Plates) and NOS, with its Kg worked out. Add as many as needed, × removes one. Under the item, <b>Planned</b> (the row's Weight when the dialog opened), <b>Returning</b> (all its lines added up) and the <b>Difference</b> update as you type — amber when less comes back than planned, red when more. Each line becomes its own excess row and its own new batch. There is no duplicate icon on a weight-only row or on a row another plan has claimed, which must come back as claimed." },
...
const ERP_MANUAL_REFERENCE_CHILDREN = [
	{
		...
		fields: [
			...
			{ name: "Inspection Entry", note: "<b>Open → Working → Completed</b>, plus <b>Feedback: Ok / Not Ok</b> on every row (on the header for a Job Card). Every row needs Feedback before Status can be set to Completed. Saving as Completed also submits, after a confirmation prompt." },
...
const ERP_MANUAL_FINISHED_GOODS_CHILDREN = [
	...
	},
	{
		...
		fields: [
			...
			{ name: "Check: the drawings add up to the line", note: "For each finished-goods line: the drawings' Totals are compared with the line's Kg and NOS. <b>Items table LESS than the drawings</b> (in Kg or Nos) stops verification — drawings would be made for more than the order sells. <b>Items table MORE than the drawings</b> (the customer's weight above the planned one) is only a warning: verification still passes, as \"Verified — with warnings\". The message reads e.g. \"planned 19091.99 Kg / 31 Nos in the Drawing List, Items table has 20000 Kg / 31 Nos — Items table has 908.01 Kg more\". On save, the same comparison shows as a small table: Row, FG Item, Planned (Drawing List), Items table, Difference." },

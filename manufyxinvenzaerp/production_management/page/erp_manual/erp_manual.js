...
const ERP_MANUAL_JOB_WORK_ORDER_CHILDREN = [
	{
		...
		steps: [
			...
			"The <b>Drawings</b> button on each row opens that operation's drawings with the weight beside every piece count — Qty to Mfg (Nos) with Planned (Kg) and Transferred (Kg), Completed (Nos) with Consumed (Kg) — and totals the columns at the foot. Consumed (Kg) is read from that operation's own Consumption Log, so it always agrees with its Total Consumed (Kg).",
		],
		buttons: [
